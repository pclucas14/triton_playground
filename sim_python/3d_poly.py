...
# Matrix dimensions
bs, n_skills, rank, d_in, d_out = 11, 1, 16, 1280, -1
seq_len = 20
...
input_ = torch.randn(bs, seq_len, d_in, dtype=DTYPE, device='cuda')
output_ = torch.empty(bs, seq_len, rank, dtype=DTYPE, device='cuda')
...
BLOCK_SIZE_K = 3
GROUP_SIZE_M = 8
BLOCK_SIZE_M = BLOCK_SIZE_N = BLOCK_SIZE_K = 16

A = torch.einsum("bs,sdr->bdr", (mixing_weights, skill_weights))
A = A.reshape(bs, d_in, rank)
exp_out = torch.einsum('bsi,bio->bso', (input_, A))
use_groups = True
...
for pid_batch in range(bs):
    for pid in range(tl_cdiv(M, BLOCK_SIZE_M) * tl_cdiv(N, BLOCK_SIZE_N)):
        ...
        print(f'pid_batch : {pid_batch}, pid_m: {pid_m}, pid_n: {pid_n}, pid: {pid}')
        assert (pid_m, pid_n) not in seen, breakpoint()
        seen[(pid_batch, pid_m, pid_n)] = True
        ...
        # input_ is bs, seq_len, d_in
        X_ptr = input_ptr + offset_am[:, None] * input_.stride(1) \
                          + offset_k[None, :] * input_.stride(2) \
                          + pid_batch * input_.stride(0)
        ...
        mix_ptr = torch.zeros(1, device=input_.device, dtype=torch.long) + \
              mixing_weights_ptr + mixing_weights.stride(0) * pid_batch # offset_am[:, None]
        ...
        for k_chunk in range(0, tl_cdiv(K, BLOCK_SIZE_K)): # makes the masking of invalid outputs easier
            ...
            mw_chunk = tl_load(mixing_weights, mix_ptr)               # BSM, n_skills          # bs,     skills       (M, _)
            ...
            # move pointers
            X_ptr += BLOCK_SIZE_K * input_.stride(2)
            Y_ptr += BLOCK_SIZE_K * skill_weights.stride(1)

        # output is bs, seq_len, d_out
        ...
        # account for strides
        O_ptr = output_ptr + output_.stride(1) * offset_om[:, None] + output_.stride(2) * offset_on[None, :]
        O_ptr += output_.stride(0) * pid_batch
