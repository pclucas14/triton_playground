...
if DEBUG: 
    config = [
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 128, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
    ]        
else:
    config=[
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 128, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 256, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 512, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 1024, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=8),    
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 2048, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 16, 'BLOCK_SIZE_K': 4096, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_K': 128, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_K': 256, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
        triton.Config({'BLOCK_SIZE_M': 32, 'BLOCK_SIZE_K': 512, 'N_SKILLS': N_SKILLS,'BLOCK_SIZE_N': 16}, num_stages=5, num_warps=4),    
...
def poly_linear_kernel(
    ...
    bs, seq_len, d_in, rank, 
    stride_alpha_b, stride_alpha_k,      # bs, n_skills 
    stride_weights_k, stride_weights_d, stride_weights_r, # n_skills, d_in
    stride_input_b, stride_input_s, stride_input_d,     # (bs), seq_len, d_in 
    stride_output_b, stride_output_s, stride_output_r,    # bs, seq_len 
    BLOCK_SIZE_M: tl.constexpr, BLOCK_SIZE_K: tl.constexpr, N_SKILLS: tl.constexpr, BLOCK_SIZE_N: tl.constexpr
):
    K = d_in
    M = bs * seq_len

    # THIS ONE IS THE ISSUE; SO WE ARE NOT INDEXING IN INPUT CORRECTLY ?  
    # MAYBE batch_idx * stride_output_b (which is a huge number) is too big
    ...
    weight_block_ptr = tl.make_block_ptr(
        base=weight_ptr,
        shape=(N_SKILLS * d_in, rank), #(N_SKILLS, d_in),
        offsets=(0, 0),
        strides=(stride_weights_d, stride_weights_r), # (stride_weights_k, stride_weights_d),
    ...
    output_block_ptr = tl.make_block_ptr(
        ...
        shape=(seq_len, rank),
        strides=(stride_output_s, stride_output_r),
        offsets=(seq_idx * BLOCK_SIZE_M, 0), #(batch_idx, seq_idx), TODO: WHY * BLOCK_SIZE_M ?
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        for skill_idx in range(N_SKILLS):
            ...
            skill_chunk = tl.load(weight_block_ptr, boundary_check=(0,1))

            weights += skill_coef * skill_chunk
            weight_block_ptr = tl.advance(weight_block_ptr, (d_in, 0))
...
def triton_poly(mixing_weights, skill_weights, X):
    ...
    n_skills, d_in, rank = skill_weights.size()
    ...
    poly_linear_kernel[grid](
        ...
        skill_weights.stride(0), skill_weights.stride(1), skill_weights.stride(2),  
...
def benchmark(M, N, K, provider):
    ...
    input = torch.randn(bs, seq_len, M, dtype=DTYPE, device=DEVICE)#.fill_(0)
     
    A = torch.einsum("bs,sdr->bdr", (mixing_weights, skill_weights))

    exp_out = torch.einsum('bsi,bir->bsr', (input, A))
    gold_out = exp_out # torch.einsum('bsi,bi->bs', (input.to(torch.float64), A.to(torch.float64))).to(DTYPE)
    triton_out = triton_poly(mixing_weights, skill_weights, input)
    show = lambda idx : ((triton_out[idx] - exp_out[idx]).pow(2) * 100).int()
    if torch.isnan(triton_out).any():
        breakpoint()
    ...
    if provider == 'einsum':
        def _fwd_pass(mixing_weights, skill_weights, input):
            A = torch.einsum("bs,sdr->bdr", (mixing_weights, skill_weights))
            return torch.einsum('bsi,bir->bsr', (input, A))
