...
def poly_linear_kernel(
    ...
    stride_output_2, stride_output_1, stride_output_0, 
    ...
    output_ptrs = output_ptr + stride_output_1 * offs_om[:, None] + stride_output_2 * offs_on[None, :]
...
def triton_poly(mixing_weights, skill_weights, X):
    ...
    output = torch.empty(bs, seq_len, rank, dtype=DTYPE, device=DEVICE).fill_(torch.nan)
    
    B, M, N = bs, seq_len, rank
    grid = lambda META: (
...
def benchmark(M, N, K, provider):
    ...
    if provider == 'einsum':
        def _fwd_pass(mixing_weights, skill_weights, input):
            A = torch.einsum("bs,sdr->bdr", (mixing_weights, skill_weights))
            return torch.einsum('bsi,bio->bso', (input, A))
